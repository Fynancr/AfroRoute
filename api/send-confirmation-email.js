...
// Sends an official Supabase continuation / resend email through Resend.
...
function continuationEmailHtml({ actionLink, replyTo }) {
  return `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#0A2540;background:#f7f9fb">
      ...
      <div style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:28px">
        <h2 style="margin:0 0 12px;font-size:21px;color:#0A2540">
          Continue to AfroRoute
        </h2>

        <p style="font-size:15px;line-height:1.6;color:#334155;margin:0 0 24px">
          Click the button below to continue to AfroRoute and access your account.
        </p>
...
async function generateMagicLinkWithSupabaseJs({ supabase, email, redirectTo }) {
...
async function generateMagicLinkWithRest({ email, redirectTo, serviceKey }) {
  const supabaseUrl = process.env.SUPABASE_URL;

  const attempts = [
    {
      label: 'magiclink_with_redirect_to',
      body: {
        type: 'magiclink',
        email,
        options: {
    ...
    {
      label: 'magiclink_with_redirectTo',
      body: {
        type: 'magiclink',
        email,
        options: {
    ...
    {
      label: 'magiclink_without_redirect',
      body: {
        type: 'magiclink',
        email,
      },
  ...
  for (const attempt of attempts) {
    ...
    console.warn('resend_generate_link_rest_attempt_failed', {
      method: attempt.label,
      status: resp.status,
      message: lastError,
    });

    if (isUserNotFoundMessage(lastError)) break;
...
module.exports = async function handler(req, res) {
  ...
  console.log('resend_confirmation_request', {
  ...
  if (keyProblem) {
    console.error('resend_service_key_problem', {
      message: keyProblem,
    });
  ...
  try {
    let result = await generateMagicLinkWithSupabaseJs({
      supabase,
      email,
      redirectTo,
    });
    ...
    if (result.error && isRedirectMessage(result.error.message)) {
      console.warn('resend_generate_link_redirect_retry_no_redirect', {
        email: maskEmail(email),
    ...
    if (result.error || !result.data?.properties?.action_link) {
      console.warn('resend_generate_link_js_failed_trying_rest', {
        email: maskEmail(email),
        message: result.error?.message || 'No action link',
      });

      result = await generateMagicLinkWithRest({
        email,
        redirectTo,
        serviceKey,
      });
    ...
    if (result.error || !actionLink) {
      console.error('resend_generate_link_failed_final', {
        email: maskEmail(email),
      ...
      if (isUserNotFoundMessage(errorMessage)) {
        return json(res, 404, {
          ...
          code: 'USER_NOT_FOUND',
          error:
            'No AfroRoute account exists for this email. Please sign up again.',
        });
      }
    ...
    if (sendError) {
      console.error('resend_email_failed', {
        email: maskEmail(email),
    ...
    console.log('resend_confirmation_sent', {
      email: maskEmail(email),
    ...
    console.error('resend_confirmation_unhandled', {
      email: maskEmail(email),
      message: err.message,
    });
    ...
    return json(res, 500, {
      success: false,
      code: 'UNHANDLED',
      error: `Could not send confirmation email. ${err.message}`,
    });
  }
};
