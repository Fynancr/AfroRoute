module.exports = async (req, res) => {
  ...
  if (!process.env.RESEND_API_KEY) {
    return res.status(500).json({ success: false, error: 'Resend API key is not configured' });
  }

  const { agreement_id, shipment_id, agreement } = req.body || {};

  if (!agreement_id) {
    return res.status(400).json({ success: false, error: 'Missing agreement_id' });
  }
  ...
  const route = agreement?.route || (agreement?.pickup_city && agreement?.delivery_city
    ? `${agreement.pickup_city} → ${agreement.delivery_city}`
    : 'Unknown route');
  ...
  const adminEmail = process.env.ADMIN_EMAIL || 'support@afroroute.com';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Resend payout notification error:', err);
      return res.status(500).json({ success: false, error: 'Failed to send admin notification' });
    }

    return res.status(200).json({ success: true, sent: true });
  } catch (err) {
    console.error('notify-payout-ready error:', { message: err.message });
    return res.status(500).json({ success: false, error: 'Failed to notify admin' });
  }
};
