...
module.exports = async (req, res) => {
  ...
  if (!email || !userId || !deviceId) {
    return res.status(400).json({ success: false, error: 'Missing email, userId, or deviceId' });
  }
  ...
  try {
    ...
    const { error: insertErr } = await supabase.from('login_verification_tokens').insert({
      user_id: userId,
    ...
    if (insertErr) {
      ...
      return res.status(500).json({ success: false, error: 'Could not create verification token' });
    }
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 20px; background: #f0f4f8;">
            ...
            <div style="background: #fff; border-radius: 12px; padding: 24px;">
              ...
              <p style="color: #475569; font-size: 14px; line-height: 1.6;">Use this code to verify it is you:</p>
              ...
              <p style="color: #94a3b8; font-size: 12px;">Need help? Contact <a href="mailto:support@afroroute.com" style="color:#1ABC9C;">support@afroroute.com</a></p>
            </div>
          </div>
        `,
      }),
    ...
    if (!response.ok) {
      ...
      console.error('Resend device verification error:', err);
      return res.status(500).json({ success: false, error: 'Failed to send verification email' });
    }

    return res.status(200).json({ success: true, sent: true });
  } catch (err) {
    console.error('Send device verification error:', { message: err.message });
